        ...
        <div className="space-y-6">
          {tasks.map((task)=>(
            <div key={task.id}
             ...
             style={{
               borderColor: 
               task.status === "Completed"? "green" : task.status === "In Progress" ? "yellow" : "red", }}
            >
              <div className='flex flex-col'>
                <h2 className='text-black text-xl'>{task.title}</h2>
                <p className="text-sm text-gray-600 mt-2">{task.description}</p>
                <p className="text-sm text-gray-500 mt-2">{task.dueDate}</p>
              </div>
               ...
               <div className="flex space-x-4">
                ...
                <button
                  className="px-4 py-2 text-gray-700 bg-red-500
